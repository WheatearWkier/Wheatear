...
namespace Wheatear::EventScriptGraphPanelInternal {
        ...
        inline static const char* InstructionTypeName(EventScriptInstructionType type)
        {
            ...
            case EventScriptInstructionType::RawLine: return "Unrecognized";
            }
            return "Unknown";
        }
        ...
        inline static ImU32 InstructionColor(EventScriptInstructionType type)
        {
            ...
            case EventScriptInstructionType::RawLine: return IM_COL32(168, 62, 58, 242);
            }
        ...
        inline static bool IsBareCommandText(const std::string& text)
        {
            const std::string lower = StringUtils::ToLower(StringUtils::Trim(text));
            return lower == "quit"
                || StringUtils::StartsWith(lower, "scene:")
                || StringUtils::StartsWith(lower, "newgame:")
                || StringUtils::StartsWith(lower, "loadgame:")
                || StringUtils::StartsWith(lower, "progression:")
                || StringUtils::StartsWith(lower, "gamesave:")
                || StringUtils::StartsWith(lower, "ui:")
                || StringUtils::StartsWith(lower, "event:");
        }
        ...
        inline static std::string SerializeEventScript(const std::vector<EventScriptBlock>& events,
            ...
            const std::vector<EventScriptInstruction>& orphanLines = {})
        {
            std::ostringstream stream;
            for (const std::string& comment : headerComments)
                stream << comment << '\n';
            if (!headerComments.empty())
                stream << '\n';

            // Preserve orphan lines (content outside any event block) in their
            // source order so nothing is lost on save.
            std::vector<EventScriptInstruction> orphans = orphanLines;
            std::sort(orphans.begin(), orphans.end(),
                [](const EventScriptInstruction& a, const EventScriptInstruction& b)
                {
                    return a.SourceLine < b.SourceLine;
                });

            auto flushOrphansBefore = [&stream](std::vector<EventScriptInstruction>& pending,
                int sourceLine)
            {
                for (auto it = pending.begin(); it != pending.end();)
                {
                    if (it->SourceLine <= 0 || it->SourceLine < sourceLine)
                    {
                        for (const std::string& comment : it->LeadingComments)
                            stream << comment << '\n';
                        stream << it->Text << '\n';
                        it = pending.erase(it);
                    }
            ...
            for (const EventScriptBlock& event : events)
            {
                ...
                flushOrphansBefore(orphans, event.SourceLine);

                for (const std::string& comment : event.LeadingComments)
                    stream << comment << '\n';
                stream << "event " << event.Name << ":\n";
                int indent = 1;
                for (const EventScriptInstruction& instruction : event.Instructions)
                {
                    for (const std::string& comment : instruction.LeadingComments)
                        WriteIndented(stream, indent, comment);
                    switch (instruction.Type)
                    {
                    case EventScriptInstructionType::Command:
                        if (!Trim(instruction.Text).empty())
                        {
                            // Non-whitelisted commands need the "command "
                            // prefix to survive a re-parse as bare text.
                            const std::string command = Trim(instruction.Text);
                            const std::string line = IsBareCommandText(command)
                                ? command
                                : "command " + command;
                            WriteIndented(stream, indent, line);
                        }
                        break;
                    ...
                    case EventScriptInstructionType::RawLine:
                        // Verbatim: no "command " prefix, no trimming, so the
                        // handwriting survives round-trips byte-for-byte.
                        if (!instruction.Text.empty())
                            WriteIndented(stream, indent, instruction.Text);
                        break;
                    }
                }
                stream << "end\n\n";
            ...
            // Any remaining orphans appeared after the last event.
            for (const EventScriptInstruction& orphan : orphans)
            {
                for (const std::string& comment : orphan.LeadingComments)
                    stream << comment << '\n';
                stream << orphan.Text << '\n';
            }

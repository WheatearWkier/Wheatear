...
namespace Wheatear {
    ...
    {
    public:
        TurnCombatTuningEditorPanel();
        ~TurnCombatTuningEditorPanel();

        TurnCombatTuningEditorPanel(const TurnCombatTuningEditorPanel&) = delete;
        TurnCombatTuningEditorPanel& operator=(const TurnCombatTuningEditorPanel&) = delete;
    ...
    private:
        ...
        // Per-unit snapshots captured when a field edit starts, so a whole
        // drag session collapses into one undo step.
        std::unordered_map<uint32_t, TurnCombatantComponent> m_UnitEditSnapshots;

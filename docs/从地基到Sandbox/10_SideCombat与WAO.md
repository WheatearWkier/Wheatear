...
## 10.12 HUD 打磨：触摸摇杆、状态图标队列（2026-08-16）

### 10.12.1 摇杆鼠标拖拽

`SideCombatHudService` 每帧检测：按住底框区域开始拖拽 → 方向量化 8 方向
（`{-1,0,1}²`，0.35 阈值）→ 经 `GetJoystickInputDirection()` 供
`SideCombatSystem` 移动采样合并（纵轴取反）。拖出底框继续有效（钳制到行程），
松开归零；未拖动时 `{0,0}`，键盘完全不受影响。

### 10.12.2 状态图标队列（FIFO + 换栏）

- buff/debuff 同属 status，合并单一队列；按**入场时间**排序（false→true 时
  记录帧时钟，持续中不重排），消失后后续前移。
- 每栏 2 个放满换栏：玩家向上、Boss 向下（远离各自血条）、小怪向上
  （头顶血条上方，实体按需运行时创建）。
- 不激活不占位；`breakDebuff` 仅认 `Broken`（保护条 0 是 Boss 常态）。

...
当前 `SkillTree.wt` 已经从文字横排改为图标化分支节点网络：
...
当前 `Equipment.wt` 已经从单装备文字说明改为装备管理页面底稿：

- 左侧显示当前装备槽，当前竖切已经直接落成武器、防具、戒指、护符、足部、特殊六个场景槽位；其中 `weapon` 显示为 `武器`，`special` 显示为 `特殊`，不再写 `副武器`。
- 这些槽位是场景里直接摆好的实体，不是运行时临时生成，方便编辑器里直接看位置和层级。
- 中央显示背包图标网格，按运行时持有装备和分页状态生成，不在场景里预占一堆空格子；当前放入旅人护衣、黑林皮甲、兽牙坠饰、初级魔晶戒、疾风短靴、旧护符、练习短剑、天使羽饰。
- 点击装备图标会写入 `GameProgress::State.SelectedEquipmentId`，并刷新 `Equipment_Details` 和材料需求。
- 分页按钮会写入 `GameProgress::State.EquipmentPage`，`Equipment_PageSlider` 用于显示页码进度；当前页码从 1 开始，页数变多时拖块移动距离也会跟着变。
- `Equipment_Details`、`Equipment_Materials` 和 `Equipment_Status` 分开显示，现阶段把字号和布局都整理过一轮，避免长文本挤出面板。

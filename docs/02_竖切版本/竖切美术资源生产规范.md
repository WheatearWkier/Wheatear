...
```text
WheatearEditor/assets/sandbox/source_art/
WheatearEditor/assets/sandbox/source_frames/
WheatearEditor/assets/sandbox/vn/
WheatearEditor/assets/sandbox/side_combat/sheets/
WheatearEditor/assets/sandbox/side_combat/vfx_sheets/
WheatearEditor/assets/sandbox/ui/atlases/
WheatearEditor/assets/sandbox/arcade_combat/sheets/
WheatearEditor/assets/sandbox/turn_combat/sheets/
```
...
归档目录名与本文档中的批次标题保持一致；运行时正式资源仍迁移到 `WheatearEditor/assets/sandbox/` 下，并由场景、脚本、数据表和 `content_manifest.yaml` 引用。`AI生成素材批次/` 只做批次追溯和原始交付保存，不作为发行包依赖路径。
...
| 类型 | 必要素材 | 正式路径/迁移策略 |
| --- | --- | --- |
| 场景 | 像素竞技场背景、危险条、障碍物/掩体 | 迁移到 `arcade_combat/*` 规范目录，旧 `assets/sandbox/arcade_combat/*` 引用清零 |
...
| 类型 | 必要素材 | 当前兼容路径/新增路径 |
| --- | --- | --- |
| 棋盘 | 遗迹战旗背景、普通格、移动格、攻击格、选中格 | `assets/sandbox/tactical_combat/backgrounds/` 与 `tiles/` |
...
| `audio.sandbox` | 主菜单、VN、战斗、Boss、据点音乐和音效 |

...
namespace Wheatear {
		...
		: m_Path(AssetPath::ToProjectRelative(path).generic_string())
	{
		...
		{
			WT_PROFILE_SCOPE("stbi_load - OpenGLTexture2D::OpenGLTexture2D(const std::string&)");
			data = stbi_load(resolvedPath.string().c_str(), &width, &height, &channels, 0);
		}
		if (!data)
		{
			WT_CORE_WARN("OpenGLTexture2D: failed to load '{}', using fallback texture", resolvedPath.string());
			m_Width = 1;
			m_Height = 1;
			m_InternalFormat = GL_RGBA8;
			m_DataFormat = GL_RGBA;

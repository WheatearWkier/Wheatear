...
namespace Wheatear {
	...
	namespace Utils {
		...
		static void CreateTextures(bool multisampled, uint32_t* outID, GLsizei count)
		{
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum internalFormat, GLenum format, uint32_t width, uint32_t height, int index)
		{
			...
			{
				glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, width, height, 0, format, TextureDataType(internalFormat), nullptr);
		...
		static GLenum WheatearFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
				case Wheatear::FramebufferTextureFormat::RGBA8:	   return GL_RGBA8;
				case Wheatear::FramebufferTextureFormat::RGBA16F:     return GL_RGBA16F;
				case Wheatear::FramebufferTextureFormat::R8:          return GL_R8;
				case Wheatear::FramebufferTextureFormat::RED_INTEGER: return GL_RED_INTEGER;
			}
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				case FramebufferTextureFormat::RGBA16F:
					Utils::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples, GL_RGBA16F, GL_RGBA, m_Specification.Width, m_Specification.Height, static_cast<int>(i));
					break;
				case FramebufferTextureFormat::R8:
					Utils::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples, GL_R8, GL_RED, m_Specification.Width, m_Specification.Height, static_cast<int>(i));
					break;
		...
		{
			WT_CORE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		WT_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!")
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			WT_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
		...
		//WT_CORE_WARN("ReadPixel at ({0}, {1}) = {2}", x, y, pixelData);
		return pixelData;
	}
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
			Utils::WheatearFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
